...
| Native mobile clients | In progress | Android and iOS now have native splash/login, secure Keychain/Keystore access and refresh sessions through the BFF, configurable localhost/ngrok API routing, catalog search/filtering, product details, customer order history, and reusable multi-line checkout API methods. Persistent cross-device cart synchronization and cart UI integration remain next. |
...
7. Implement persistent cross-device cart state and server checkout on both native clients,
