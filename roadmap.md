...
| Additional domain services | In progress | Product, Inventory, and Order Services are deployed and healthy in the local Kind cluster. Order Service now consumes published Product and Inventory gRPC contract modules, coordinates price snapshots and reservations, is wired through Helm dependency addresses ([PR #5](https://github.com/sartim/storemesh-helm-repo/pull/5)), supports idempotent creation with PostgreSQL-backed retries, and has an in-process gRPC integration test. Deployment-level tests against the Kind services remain next. |
