...
| Native mobile clients | In progress | Android and iOS now have native splash/login, persisted access and refresh sessions through the BFF, configurable localhost/ngrok API routing, catalog search/filtering, product details, and customer order history. Shared cart state, server checkout, and mobile integration tests remain next. |
...
7. Implement shared cart state and server checkout on both native clients,
