...
| Additional domain services | In progress | Product, Inventory, and Order Services are deployed and healthy in the local Kind cluster. Order Service now consumes published Product and Inventory gRPC contract modules, coordinates price snapshots and reservations, and is wired through Helm dependency addresses ([PR #5](https://github.com/sartim/storemesh-helm-repo/pull/5)). Cross-service workflow integration tests and idempotency remain next. |
