...
| Native mobile clients | In progress | Android now has splash, persisted login through the BFF, emulator-localhost configuration, a responsive customer drawer, product search, and catalog filters. iOS has the generated native SwiftUI project plus the StoreMesh foundation; checkout, order history, and iOS auth/catalog remain next. |
