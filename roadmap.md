...
| Additional domain services | In progress | Product and Inventory Services are deployed and healthy; Order Service contract, lifecycle runtime, PostgreSQL schema, transactional persistence, container workflow, Helm chart ([PR #4](https://github.com/sartim/storemesh-helm-repo/pull/4)), and Argo CD application ([PR #3](https://github.com/sartim/storemesh-argocd-repo/pull/3)) are available. Local image publication and workload health verification remain before cross-service coordination. |
| Edge composition / BFF | Deferred | Evaluate after client journeys demonstrate cross-service aggregation or client-specific composition; choose REST or GraphQL for the edge API based on those client needs while using internal gRPC contracts |
