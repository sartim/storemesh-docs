...
| Native mobile clients | Planned | Native Kotlin/Jetpack Compose Android and Swift/SwiftUI iOS foundations are created; next is shared REST contract integration, authentication, catalog, checkout, and order history on each platform. |
...
7. Build the Android and iOS catalog journeys independently using native
   platform patterns, then add checkout and order history after authentication
   and API error handling are stable.
